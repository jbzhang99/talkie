...
import com.meta.dao.qmanage.QEnterpriseRepository;
...
@Service
@Transactional
public class QEnterpriseService   extends BaseServiceImpl<QUser, Long> implements BaseService<QUser, Long> {
    ...
    @Autowired
    public void setBaseDao(QEnterpriseRepository qEnterpriseRepository) {
        super.setBaseDao(qEnterpriseRepository);
    }

    @Autowired
    private QEnterpriseRepository qEnterpriseRepository;
    ...
    public boolean modifyBalanceByid(Long id,Double balance) {
        boolean flag = false;
        try {
            qEnterpriseRepository.modifyBalanceByid(id, balance);
            flag = true;
        } catch (Exception e) {
    ...
    public QUser findByUserID(Long userid) {
        return qEnterpriseRepository.findByUser_Id(userid);
    }
}
